...
function VendorTermsAndCondition() {
  ...
  const getSupportDetailsFunc = async () => {
    try {
      ...
      if (response?.data?.data) {
        setContent(response.data.data.vendorTermsAndCondition || "");
        contentRef.current = response.data.data.vendorTermsAndCondition || "";
  ...
  const handleSupportUpdateDetails = async () => {
    ...
    setBtnLoader(true);
    try {
      const response = await updateSupportDetailsServ({
        _id,
        vendorTermsAndCondition: contentRef.current,
      });
            ...
            {staticsArr?.map((v, i) => {
              return (
                <div className="col-md-4 col-12 ">
                  <div
                    className={
                      "topCard  py-4 px-3 rounded mb-3 " +
                      (v?.title == "Vendor" ? " shadow border " : " shadow-sm")
                    }
...
export default VendorTermsAndCondition;
