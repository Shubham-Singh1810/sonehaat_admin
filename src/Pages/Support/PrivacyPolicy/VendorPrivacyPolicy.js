...
function VendorPrivacyPolicy() {
  ...
  const getSupportDetailsFunc = async () => {
    try {
      ...
      if (response?.data?.data) {
        setContent(response.data.data.vendorPrivacyPolicy || "");
        contentRef.current = response.data.data.vendorPrivacyPolicy || "";
  ...
  const handleSupportUpdateDetails = async () => {
    ...
    setBtnLoader(true);
    try {
      const response = await updateSupportDetailsServ({ _id, vendorPrivacyPolicy: contentRef.current });
            ...
            {staticsArr?.map((v, i) => {
              return (
                <div className="col-md-4 col-12 ">
                  <div onClick={()=>navigate(v?.path)} className={"topCard  py-4 px-3 rounded mb-3 " + (v?.title=="Vendor" ? " shadow border ":" shadow-sm")}>
