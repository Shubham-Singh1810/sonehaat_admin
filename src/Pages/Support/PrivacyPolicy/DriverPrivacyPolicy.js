...
function DriverPrivacyPolicy() {
  ...
  const getSupportDetailsFunc = async () => {
    try {
      ...
      if (response?.data?.data) {
        setContent(response.data.data.driverPrivacyPolicy || "");
        contentRef.current = response.data.data.driverPrivacyPolicy || "";
  ...
  const handleSupportUpdateDetails = async () => {
    ...
    setBtnLoader(true);
    try {
      const response = await updateSupportDetailsServ({ _id, driverPrivacyPolicy: contentRef.current });
