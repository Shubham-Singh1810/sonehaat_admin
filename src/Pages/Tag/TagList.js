...
function TagList() {
  ...
  const handleGetCategoryFunc = async () => {
    ...
    try {
      let response = await getTagSetServ(payload);
  ...
  const handleAddCategoryFunc = async () => {
    setIsLoading(true);
    try {
      let response = await addTagServ(addFormData);
  ...
  const handleDeleteCategoryFunc = async (id) => {
    ...
    if (confirmed) {
      try {
        let response = await deleteTagServ(id);
        if (response?.data?.statusCode == "200") {
          toast?.success(response?.data?.message);
          handleGetCategoryFunc();
        }
        ...
        <div className="p-lg-4 p-md-3 p-2">
          ...
          <div className="row m-0 p-0 d-flex align-items-center my-4 topActionForm">
            <div className="col-lg-2 mb-2 col-md-12 col-12">
              <h3 className="mb-0 text-bold text-secondary">Tags</h3>
            </div>
