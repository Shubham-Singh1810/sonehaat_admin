...
function TicketCategoryList() {
  ...
  const handleAddCategoryFunc = async () => {
    setIsLoading(true);
    try {
      let response = await ticketAddServ(addFormData);
  ...
  const handleDeleteCategoryFunc = async (id) => {
    ...
    if (confirmed) {
      try {
        let response = await ticketDeleteServ(id);
        if (response?.data?.statusCode == "200") {
          toast?.success(response?.data?.message);
          handleGetCategoryFunc();
        }
  ...
  const handleUpdateCategoryFunc = async () => {
    ...
    try {
      let response = await ticketUpdateServ(editFormData);
      if (response?.data?.statusCode == "200") {
        toast.success(response?.data?.message);
        setEditFormData({
          name: "",
  ...
  return (
    <div className="bodyContainer">
      <Sidebar
        ...
        selectedItem="Ticket Categories"
      />
